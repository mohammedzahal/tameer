    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            className={`absolute top-4 ${isRtl ? 'left-4' : 'right-4'} p-2.5 rounded-full bg-black/70 border border-white/10 text-zinc-400 hover:text-white hover:border-white/30 transition-colors z-30`}
          >
            <X className="w-4 h-4" />
          </button>

          {/* Full-Frame Photographic Canvas */}
          <div className="w-full h-[65vh] sm:h-[75vh] rounded-lg overflow-hidden border border-zinc-900 bg-zinc-950 relative flex items-center justify-center">
            <img
              src={project.image}
              alt={project.title}
